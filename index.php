...
//$script_key = '7c3563b8892e00aa71c9b256d66c8852';
...
for(var i = 0; i <= options.length; i++) {
    if(options[i] != undefined)
        ...
        if(isset($this->query))
        {
			...
			if($this->clear_url == 'single.page.ajax') // for apishops
				$ext = 'css';
    ...
    public function setHeaders() {
		...
		if($this->clear_url == 'single.page.ajax')
...
if(isset($_GET['sitecopy']) and isset($_GET['image_example'])) {
    if(!file_exists('temp')) {
        mkdir('temp');
        $http = new http;
        //$img = $http->get('system/657fb47249d61a96d30831c02fa82d76.jpg');
...
elseif(!file_exists('config.cfg')) {
		...
		if(!isset($_GET['url']))
			...
			if(preg_match('#</head>#', $html)) {
				...
				$html = preg_replace('#'.$tag.'#', 
									strrev('<script src="/system/jquery-1.11.1.min.js"></script>'.
									...
									'<link rel="stylesheet" href="/system/bw.css" />').$tag, strrev($html), 1);
				$html = strrev($html);
    ...
    elseif(isset($_POST['go'])) // Установка
    {
		...
		if(isset($config['fh_forms']) and isset($config['fh_handlers'])) {
			$handlers = json_decode($config['fh_handlers']);
			$forms = json_decode($config['fh_forms']);
			
			$config['forms'] = array();
			$forms = (array)$forms;
			foreach($forms as $pointName => $form_data) {
				$form_data = (array)$form_data;
				$config['forms'][] = array('pointName' => $pointName, 'pointType' => $form_data['pointType'], 'handler' => $form_data['handler']);
			}
			
			foreach((array)$handlers as $handler_name => $handler_data) {
				$handler_data = (array)$handler_data;
				if($handler_data['type'] == 'email') {
					$handler_data['emailsend_text'] = str_replace("'", '"', $handler_data['emailsend_text']);
					$handler_data['emailsend_text'] = "'" . preg_replace('#\{\{name_([^}]+)}}#', '\'.@$_POST[\'$1\'].\'', $handler_data['emailsend_text']) . "'";
					$handler_data['emailsend_text'] = str_replace('{{datetime}}', '\'.date("Y-m-d H:i:s").\'', $handler_data['emailsend_text']);
					file_put_contents('cache/schandler_'.$handler_name.'.php',
'<?!!php
header("Content-Type: text/html; charset=utf-8");
if($_SERVER["REQUEST_METHOD"] == "GET")
	exit;
	
$email = "'.$handler_data['emailsend_email'].'";
$title = "'.$handler_data['emailsend_title'].'";
$text = '.$handler_data['emailsend_text'].';
...
!!?>');
					unset($handler_data['getpostsend_name']);
					unset($handler_data['getpostsend_type']);
					unset($handler_data['getpostsend_url']);
				}
				elseif($handler_data['type'] == 'getpostsend') {
					if($handler_data['getpostsend_type'] == 2)
						$handler_data['getpostsend_type'] = 'post';
					elseif($handler_data['getpostsend_type'] == 1)
						$handler_data['getpostsend_type'] = 'get';
						
					unset($handler_data['emailsend_name']);
					unset($handler_data['emailsend_email']);
					unset($handler_data['emailsend_title']);
					unset($handler_data['emailsend_text']);
				}
				
				unset($handler_data['handlerlist']);
				unset($handler_data['name']);
				$config['handlers'][$handler_name] = $handler_data;
			}
			
			unset($config['fh_forms']);
			unset($config['fh_handlers']);
		}
